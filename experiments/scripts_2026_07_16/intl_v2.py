"""International with adjusted train/val/test for intl data."""
import numpy as np
import pandas as pd
import json, os
import torch
...
# For intl: use 2010-2014 train, 2015-2019 val, 2020-2024 H1
train_start = pd.Timestamp('2010-01-01')
train_end = pd.Timestamp('2016-12-31')
val_end = pd.Timestamp('2019-12-31')
h1_start = pd.Timestamp('2020-01-01')
h1_end = pd.Timestamp('2024-12-31')
...
for ticker in INTL_TICKERS:
    ...
    is_train = (seq_dates >= train_start) & (seq_dates <= train_end)
    is_val = (seq_dates > train_end) & (seq_dates <= val_end)
    is_test_h1 = (seq_dates >= h1_start) & (seq_dates <= h1_end)
    is_train_h2 = seq_dates < h2_start
    is_test_h2 = (seq_dates >= h2_start) & (seq_dates <= h2_end)
    if is_train.sum() < 500 or is_val.sum() < 100 or is_test_h1.sum() < 30: continue
    ...
    print(f"  {ticker}: H1={intl_results['H1'].get(ticker, 'N/A')}", flush=True)
